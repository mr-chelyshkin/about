...
resource "aws_cloudfront_function" "static_uri_rewrite" {
  name    = "${local.policy_name_prefix}-uri-v1"
  comment = "Resolve clean static routes to S3 index objects"
  runtime = "cloudfront-js-2.0"
  publish = true
  code    = file("${path.module}/functions/static-uri-rewrite.js")
}

resource "aws_cloudfront_cache_policy" "site" {
  name        = "${local.policy_name_prefix}-cache-v1"
  comment     = "Honor origin cache headers and explicit asset version queries"
  default_ttl = 0
  max_ttl     = 31536000
  min_ttl     = 0
...
resource "aws_cloudfront_distribution" "site" {
  enabled             = true
  comment             = "Igor Chelyshkin personal site"
  ...
  default_cache_behavior {
    ...
    function_association {
      ...
      function_arn = aws_cloudfront_function.static_uri_rewrite.arn
    }
  }
