...
export function useImageGlitch(options: UseImageGlitchOptions = {}) {
  ...
  const triggerGlitch = () => {
    ...
    glitchTimeoutId = window.setTimeout(() => { isGlitching.value = false; }, duration);
  };
  const scheduleNextGlitch = () => {
    const delay = getRandomDelay(minDelay, maxDelay);
    scheduleTimeoutId = window.setTimeout(() => { triggerGlitch(); scheduleNextGlitch(); }, delay);
  };
  const startAutoGlitch = () => {
    const initialDelay = getRandomDelay(initialMinDelay, initialMaxDelay);
    scheduleTimeoutId = window.setTimeout(() => { triggerGlitch(); scheduleNextGlitch(); }, initialDelay);
  };
  ...
  onMounted(() => { if (autoStart) { startAutoGlitch(); }});
  onUnmounted(() => { stopAutoGlitch(); });
