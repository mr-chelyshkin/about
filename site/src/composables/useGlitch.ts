...
export function useGlitch(options: UseGlitchOptions = {}) {
  ...
  const trigger = () => {
    ...
    setTimeout(() => { isGlitching.value = false; }, duration);
  };

  const glitchingItems = ref<Set<number>>(new Set());
  const triggerByIndex = (index: number) => {
    glitchingItems.value.add(index);
    setTimeout(() => { glitchingItems.value.delete(index); }, duration);
  };

  const isGlitchingByIndex = (index: number) => { return glitchingItems.value.has(index); };
  const createWatcher = <T>(watchTarget: Ref<T> | (() => T), compareWith?: Ref<T>) => {
    const prevValue = ref<T>();
    return (newValue: T) => {
      if (prevValue.value !== undefined && prevValue.value !== newValue) { trigger(); }
